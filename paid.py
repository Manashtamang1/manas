...
def reg():
    ...
    jalan('  ESEWA ACCOUNT NO (9820635880) ')
    jalan('  ESEWA ACCOUNT NAME (K****** RUMBA) ')
    ...
    try:
        to = open('/sdcard/.mus.txt', 'r').read()
    except (KeyError, IOError):
        reg2()

    r = requests.get('https://github.com/Manashtamang1/Approval/blob/main/approval.txt').text
    if to in r:
        ...
        os.system('xdg-open https://wa.me/+9779844255558')
        reg()
...
def seved_ok_cp(ok, cp):
    if len(ok) != 0 or len(cp) != 0:
        ...
        print '[%s+%s] total OK : %s%s%s' % (O, N, H, str(MT(ok)), N)
        print '[%s+%s] total CP : %s%s%s' % (O, N, K, str(MM(cp)), N)
        print '___________________________________________________________'
...
class __crack__:
            ...
            while True:
                ...
                def __yan__(ysc = (None, None)):
                    ...
                    if cin == '':
                        ...
                        print '\n [%s+%s] manas OK file -> results/OK-%s-%s-%s.txt' % (O, N, ha, op, ta)
                        print ' [%s+%s] tamang CP file-> results/CP-%s-%s-%s.txt' % (O, N, ha, op, ta)
    ...
    def __pler__(self):
        ...
        if yan == '':
            ...
            with manas__pass(max_workers = 30) as __mawan__:
                for yntkts in self.id:
